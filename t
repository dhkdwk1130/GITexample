...
DDEESSCCRRIIPPTTIIOONN
       ...
       time you push into it, by setting up _h_o_o_k_s there. See
       ...
       configuration to decide what to push (See ggiitt--ccoonnffiigg(1) for the
       ...
       to the simple value for push.default: the current branch is
...
OOPPTTIIOONNSS
       ...
       <refspec>...
           ...
           push, but it can be any arbitrary "SHA-1 expression", such
           as master~4 or HEAD (see ggiittrreevviissiioonnss(7)).

           The <dst> tells which ref on the remote side is updated
           with this push. Arbitrary expressions cannot be used here,
           an actual ref must be named. If git push [<repository>]
           without any <refspec> argument is set to update some ref at
           the destination with <src> with remote.<repository>.push
           configuration variable, :<dst> part can be omitted—such a
           push will update a ref that <src> normally updates without
           any <refspec> on the command line. Otherwise, missing
           ...
           fast-forward.) This does nnoott attempt to merge <src> into
       ...
       --prune
           ...
           refs/tmp/foo will be removed if refs/heads/foo doesn’t
           exist.
       ...
       --porcelain
           ...
           each ref will be tab-separated and sent to stdout instead
           of stderr. The full symbolic names of the refs will be
           given.
       ...
       --follow-tags
           ...
           specified with configuration variable _p_u_s_h_._f_o_l_l_o_w_T_a_g_s. For
           more information, see _p_u_s_h_._f_o_l_l_o_w_T_a_g_s in ggiitt--ccoonnffiigg(1).
       ...
       --[no-]signed, --sign=(true|false|if-asked)
           ...
           See ggiitt--rreecceeiivvee--ppaacckk(1) for the details on the receiving
           end.
       ...
       --receive-pack=<git-receive-pack>, --exec=<git-receive-pack>
           Path to the _g_i_t_-_r_e_c_e_i_v_e_-_p_a_c_k program on the remote end.
       ...
       --force-with-lease=<refname>:<expect>
           ...
           on top of your original history while you are rebasing, the
           tip of the branch at the remote may advance with her
           commit, and blindly pushing with --force will lose her
           work.

           This option allows you to say that you expect the history
           you are updating is what you rebased and want to replace.
           If the remote ref still points at the commit you specified,
           you can be sure that no other people did anything to the
           ref. It is like taking a "lease" on the ref without
           explicitly locking it, and the remote ref is updated only
           ...
           --force-with-lease=<refname>, without specifying the
           ...
           --force-with-lease=<refname>:<expect> will protect the
           named ref (alone), if it is going to be updated, by
           requiring its current value to be the same as the specified
           value <expect> (which is allowed to be different from the
           remote-tracking branch we have for the refname, or we do
           not even have to have such a remote-tracking branch when
           this form is used).
       ...
       -f, --force
           ...
           not an ancestor of the local ref used to overwrite it.
           Also, when --force-with-lease option is used, the command
           refuses to update a remote ref whose current value does not
           ...
           local refs that are strictly behind their remote
           counterpart). To force a push to only one branch, use a +
           in front of the refspec to push (e.g git push origin
           +master to force a push to the master branch). See the
       ...
       -u, --set-upstream
           ...
           ggiitt--ppuullll(1) and other commands. For more information, see
           _b_r_a_n_c_h_._<_n_a_m_e_>_._m_e_r_g_e in ggiitt--ccoonnffiigg(1).

       --[no-]thin
           These options are passed to ggiitt--sseenndd--ppaacckk(1). A thin
       ...
       --recurse-submodules=check|on-demand|no
           ...
           branch. If _c_h_e_c_k is used Git will verify that all submodule
           ...
           non-zero status. If _o_n_-_d_e_m_a_n_d is used all submodules that
           changed in the revisions to be pushed will be pushed. If
           on-demand was not able to push all necessary revisions it
           will also be aborted and exit with non-zero status. A value
           of _n_o or using _-_-_n_o_-_r_e_c_u_r_s_e_-_s_u_b_m_o_d_u_l_e_s can be used to
...
GGIITT UURRLLSS
       ...
       when the former implies --local option. See ggiitt--cclloonnee(1) for
       details.
...
RREEMMOOTTEESS
       ...
       command line because they each contain a refspec which git will
       use by default.
   ...
   NNaammeedd rreemmoottee iinn ccoonnffiigguurraattiioonn ffiillee
       ...
       even by a manual edit to the $GIT_DIR/config file. The URL of
   ...
   NNaammeedd ffiillee iinn $$GGIITT__DDIIRR//rreemmootteess
       ...
       $GIT_DIR/remotes. The URL in this file will be used to access
       ...
       Push: lines are used by _g_i_t _p_u_s_h and Pull: lines are used by
       _g_i_t _p_u_l_l and _g_i_t _f_e_t_c_h. Multiple Push: and Pull: lines may be
...
OOUUTTPPUUTT
       ...
       summary
           ...
           an argument to git log (this is <old>..<new> in most cases,
           ...
           remote rejected
               ...
               effect: receive.denyCurrentBranch (for pushes to the
               checked out branch), receive.denyNonFastForwards (for
               forced non-fast-forward updates), receive.denyDeletes
...
NNOOTTEE AABBOOUUTT FFAASSTT--FFOORRWWAARRDDSS
       ...
       that used to point at commit A to point at another commit B, it
       is called a fast-forward update if and only if B is a
       descendant of A.
       ...
       commit X, and you built a history leading to commit B while the
       other person built a history leading to commit A. The history
       looks like this:
       ...
       (that now points at A) with commit B. This does _n_o_t
       fast-forward. If you did so, the changes introduced by commit A
       will be lost, because everybody will now start building on top
       of B.
       ...
       words, "git push --force" is a method reserved for a case where
...
EEXXAAMMPPLLEESS
       ...
       git push origin
           ...
           to the configured upstream (remote.origin.merge
           ...
           to origin use git config remote.origin.push HEAD. Any valid
       ...
       git push mothership master:satellite/master dev:satellite/dev
           ...
           refs/remotes/satellite/master) in the mothership
           ...
           integrate the work done on satellite, and is often
       ...
       git push origin HEAD:master
           ...
           in the origin repository. This form is convenient to push
       ...
       git push origin master:refs/heads/experimental
           ...
           the local name and the remote name are different;
           otherwise, the ref name on its own will work.

       git push origin :experimental
           Find a ref that matches experimental in the origin
           repository (e.g.  refs/heads/experimental), and delete it.

       git push origin +dev:master
           Update the origin repository’s master branch with the dev
           branch, allowing non-fast-forward updates.  TThhiiss ccaann lleeaavvee
           uunnrreeffeerreenncceedd ccoommmmiittss ddaanngglliinngg iinn tthhee oorriiggiinn rreeppoossiittoorryy..
